...
cat = np.loadtxt(text_file_dir + '/polygon_cat.txt')
cat = cat + 1

v_id = np.loadtxt(text_file_dir + '/v_id_per_face_polygon.txt').astype('int')
v_id = v_id + 1
...
np.savetxt(text_file_dir + '/faces_info.txt',faces_info,'%s',delimiter='')  
...
np.savetxt(text_file_dir + '/vertices_info.txt',vertex_info,fmt = '%s',delimiter = '')


"""

"""
with open(text_file_dir + '/vertices_info.txt','r') as v_info:
    v_i = v_info.readlines()
with open(text_file_dir + '/faces_info.txt','r') as f_info:
...
output_file_name = mesh_file_dir + '/test.k'
